...
A static, single-page-per-tool study site for the GRE General Test: vocabulary flashcards, two vocab game modes, quant concept flashcards, and six full timed practice sets for Verbal and Quantitative Reasoning (including three Hard sets), with a full answer key and explanations. Includes light gamification (daily streak, XP, levels) using `localStorage` — no backend, no build step, no dependencies beyond two Google Fonts.
...
- Each quiz page keeps its own history of the last 5 attempts per practice set in `localStorage` (separate keys for Verbal and Quant), including score, set number, and full per-question answers so a past attempt's review screen can be reconstructed later.
